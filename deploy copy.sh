...
#  GPU exporter runs natively on Windows (outside Docker).
#
#  Usage:
...
#    ./deploy.sh gpu        # start GPU exporter on Windows (native)
...
show_status() {
  ...
  echo ""
  echo "  # terminal B — test model stream:"
  echo "  curl -N -X POST http://localhost:8080/predict/model/stream \\"
  echo "    -H 'Content-Type: application/json' \\"
  echo "    -d '{\"prompt\":\"hello, who are you?\"}'"
}
